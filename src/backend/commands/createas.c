...
typedef struct
{
	...
	TupleTableSlot *mi_slots[MAX_MULTI_INSERT_TUPLE_NUM]; /* buffered slots for a multi insert batch. */
...
intorel_startup(DestReceiver *self, int operation, TupleDesc typeinfo)
{
	...
	memset(myState->mi_slots, 0, sizeof(TupleTableSlot *) * MAX_MULTI_INSERT_TUPLE_NUM);
...
intorel_shutdown(DestReceiver *self)
{
	...
	for (i = 0; i < MAX_MULTI_INSERT_TUPLE_NUM && myState->mi_slots[i] != NULL; i++)
